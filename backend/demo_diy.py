...
async def test_diy_provision_and_multi_plant():
    async with async_session_factory() as db:
        ...
        if not user:
            user = User(google_id="test_g_id", email="test@test.com", display_name="Test User")
            db.add(user)
            await db.flush()
        ...
        if not plant_type:
            plant_type = PlantType(name="Sen Đá", soil_moisture_min=10, soil_moisture_max=50, light_min=100, light_max=1000, temperature_min=20, temperature_max=30, humidity_min=30, humidity_max=70)
            db.add(plant_type)
            await db.flush()
        ...
        plant1 = await pair_plant(db, user, device_info['plant_code'], device_info['verify_code'], "Cây 1", plant_type.id)
        print(f"Đã liên kết Cây 1: {plant1.name}, Quality: {plant1.current_overall_quality}")

        # Cấp mã 2
        device_info_2 = await provision_device(db)
        
        # Liên kết cây 2 (sẽ không bị lỗi 1 người 1 cây nữa)
        plant2 = await pair_plant(db, user, device_info_2['plant_code'], device_info_2['verify_code'], "Cây 2", plant_type.id)
        print(f"Đã liên kết Cây 2: {plant2.name}, Quality: {plant2.current_overall_quality}")
