...
async def get_or_create_dev_user(email: str, role: str) -> User:
    async with async_session_factory() as db:
        ...
        if user is None:
            ...
            await db.flush()
            await db.commit()
            print(f"✅ Đã tạo mới tài khoản Dev User:")
            print(f"   - Email: {email}")
            print(f"   - Role: {role}")
            ...
            if user.role != role:
                ...
                print(f"ℹ️ Tìm thấy tài khoản Dev User có sẵn:")
                print(f"   - Email: {email}")
                print(f"   - Role: {role}")
...
def main():
    parser = argparse.ArgumentParser(description="Sinh JWT Access & Refresh Token cho Mộc Đạo Tu Tiên.")
    parser.add_argument("--email", type=str, default="dev-admin@moctu.com", help="Email của tài khoản cần sinh token.")
    parser.add_argument("--role", type=str, default="admin", choices=["user", "admin"], help="Quyền của tài khoản.")
    ...
    print("4. Trong Postman, chọn tab 'Authorization' -> Type: 'Bearer Token' -> Dán token vào.")
    print("="*80)
